...
export function HomeSliderForm({ slider, onSubmit, onCancel, loading }: HomeSliderFormProps) {
  const { t } = useLanguage();
  const [formData, setFormData] = useState({
    titleUz: slider?.titleUz || "",
    titleRu: slider?.titleRu || "",
    subtitleUz: slider?.subtitleUz || "",
  ...
  const [imagePreview, setImagePreview] = useState<string>(slider?.coverImage || "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.titleUz || !formData.titleRu || !formData.coverImage) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setFormData(prev => ({ ...prev, subtitleUz: e.target.value }))}
                required
              />
            </div>
            <div className="space-y-2">
            ...
            <div className="flex items-center gap-4">
              {imagePreview ? (
                <div className="relative w-32 h-32 border rounded-lg overflow-hidden">
                  <S3Image src={imagePreview} alt="Slider preview" fill className="object-cover" />
                ...
                <button type="button" onClick={() => setImageModalOpen(true)} className="w-32 h-32 border-2 border-dashed rounded-lg flex flex-col items-center justify-center cursor-pointer hover:border-primary">
                  <ImageIcon className="w-8 h-8 text-muted-foreground mb-2" />
                  <span className="text-xs text-muted-foreground">{t("Tanlash", "Выбрать")}</span>
                </button>
              )}
              {imagePreview && (
          ...
          <div className="space-y-2">
            ...
            <Input id="order" type="number" value={formData.order} onChange={(e) => setFormData(prev => ({ ...prev, order: parseInt(e.target.value) || 0 }))} />
          </div>
          ...
          <div className="flex gap-3">
            <Button type="submit" disabled={loading} className="flex-1">
              {loading ? t("Saqlanmoqda...", "Сохранение...") : slider ? t("Yangilash", "Обновить") : t("Qo'shish", "Добавить")}
            </Button>
            <Button type="button" variant="outline" onClick={onCancel} disabled={loading}>{t("Bekor qilish", "Отмена")}</Button>
          </div>
